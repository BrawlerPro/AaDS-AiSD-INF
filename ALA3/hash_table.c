...
int insEl(Table *table, unsigned int key, const char *info) {
    unsigned int index = hash1(key, table->capacity);
    unsigned int step = hash2(key, table->capacity);
    for (int i = 0; i < table->capacity; i++) {
        if (table->ks[index].key == key && table->ks[index].busy == 1) return 1;
...
int resizeTable(Table *table) {
    ...
    Table *newTable = initTable(newSize);
    for(int i = 0; i < table->capacity; i++){
        if(table->ks[i].busy == 1){
            insEl(newTable, table->ks[i].key, table->ks[i].info);
        }
    }
    frTab(table);
...
UsKeySP *searchElement(struct Table *table, unsigned int key) {
    unsigned int index = hash1(key, table->capacity);
    unsigned int step = hash2(key, table->capacity);
    for (int i = 0; i < table->capacity; i++) {
        if (table->ks[index].key == key && table->ks[index].busy == 1) {
            UsKeySP *foundElement = (UsKeySP *) malloc(sizeof(UsKeySP));
            if (foundElement == NULL)
                return NULL;
            foundElement->key = table->ks[index].key;
            foundElement->info = strdup(table->ks[index].info);
...
int exportTable(Table *table, const char *filename) {
    ...
    for (int i = 0; i < table->capacity; i++) {
        if(table->ks[i].busy == 1){
            ...
            int infoLength = (int) strlen(table->ks[i].info);
...
int importTable(Table *table, const char *filename) {
    ...
    Table *newTable = initTable(capacity);
    ...
    for (int i = 0; i < size; i++) {
        ...
        info = (char *) malloc((infoLength + 1) * sizeof(char));
        fread(info, sizeof(char), infoLength, file);
        info[infoLength] = '\0';

        insertElement(newTable, key, info);
