...
void calculateAndEnqueue(Queue *master_queue, Queue *slave_queue) {
    int angle;
    double data;
    while (!isEmpty(slave_queue)) {
        dequeue(slave_queue, &data, &angle);
        enqueue(master_queue, calculateSin(angle), angle);
    }
}

// С вероятностью p3 ведущий узел вычитывает результаты выполнения заданий ведомыми узлами и добавляет
// их в итоговую таблицу.
void printResults(Queue *master_queue) {
    double result;
    int angle;
    while (!isEmpty(master_queue)){
        dequeue(master_queue, &result, &angle);
